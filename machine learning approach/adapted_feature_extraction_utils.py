...
def get_intensity_attributes(sound, time_step=0., min_time=0., max_time=0., pitch_floor=75.,
                             ...
                             replacement_for_nan=0.): # tava em 75
    """
    ...
    #attributes['min_intensity'] 
    min_intensity = call(intensity, 'Get minimum',
                                       min_time, max_time,
    ...
    #attributes['max_intensity'] 
    max_intensity = call(intensity, 'Get maximum',
                                       min_time, max_time,
                                       interpolation_method)
    ...
    #attributes['mean_intensity'] 
    mean_intensity = call(intensity, 'Get mean',
                                        min_time, max_time)
...
def get_utterance_avg_pitch(sound, pitch_type='preferred', time_step=0.02, min_time=0., max_time=0.,
       ...
       mean_pitch  = call(pitch, 'Get mean',
                                    min_time, max_time,
                                    unit)
       return mean_pitch
...
def get_pitch_attributes(sound, pitch_type='preferred', time_step=0.001, min_time=0., max_time=0.,
    ...
    #print(f"Sound starts at: {sound.xmin:.3f}s")
    #print(f"Sound ends at: {sound.xmax:.3f}s")
    ...
    if num_voiced_frames == 0:#math.isnan(max_pitch):
    ...
    frame_times = [call(pitch, "Get time from frame number", i) for i in range(1, num_frames+1)]
    pitch_values = [call(pitch, "Get value in frame", i, "Hertz") for i in range(1, num_frames+1)]
    #for time, pitch_value in zip(frame_times, pitch_values):
       #print(f"Time: {time:.3f}s - Pitch: {pitch_value:.2f} Hz")
    
    #if pitch is None:
    #   print("Error: Pitch object was not created.")
    #else: 
    #   print("Pitch object was successfully created")
       #print(pitch)
    ...
    #print("f0_range", max_pitch - min_pitch)
    ...
    #print("f0_maxavg_diff", attributes['f0_maxavg_diff'])
    #print("f0_avgmin_diff", attributes['f0_avgmin_diff'])

    #if math.isnan(attributes['f0_maxavg_diff']):
    #   print("NAN")
    #   quit() 
    #pitch_values = None

    #if return_values:
    #   pitch_values = [call(pitch, 'Get value in frame', frame_no, unit) for frame_no in range(len(pitch))]
        # Convert NaN values to floats (default: 0)
    #   pitch_values = [value if not math.isnan(value) else replacement_for_nan for value in pitch_values]
