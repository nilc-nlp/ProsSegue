...
def extract_prosody(frame, sr, frame_id, next_interval_text, next_interval_dur, interval_start_time, interval_end_time, nucleus_end_time, nucleus_start_time,  nucleus_vowel, vowel_stats_dict, utt_avg_pitch):
  ...
  if nucleus_vowel != None:
    ...
    if vowel_type_std_dev != 0: # to avoid float division by 0
      attributes['n_dur'] = ((nucleus_end_time - nucleus_start_time) - vowel_type_mean) / vowel_type_std_dev 
    else:
      attributes['n_dur'] = (nucleus_end_time - nucleus_start_time)
  else: 
...
def run_pipeline(audio_path, tg_phone, filename, tg_reference=[]):
  ...
  for vowel_type in vowel_stats_dict:
    if vowel_stats_dict[vowel_type]['durations'] != []:
  ...
  if tg_reference != []:
    ...
    # CHOOSE THE CORRECT TB_tiers list according to the corpus
    TB_tiers = [tier for tier in tg_reference.tiers if tier.name.startswith("TB")] # CORAL BRASIL
    #TB_tiers = [tier for tier in tg_reference.tiers if tier.name.startswith("TB-") and "ponto" not in tier.name] # MUPE DIVERSIDADES
    all_utterances = []
